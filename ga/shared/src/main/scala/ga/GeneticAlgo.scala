...
object GeneticAlgo {
  ...
  def createNextGeneration[A: AlgoSettings](
    ...
    val (newSeed, newGeneration) = originalPopulation.zipWithIndex.foldLeft(initialSeed -> IndexedSeq.empty[Geneology[A]]) {
      case ((nextSeed, results: IndexedSeq[Geneology[A]]), (mom, i)) =>
        ...
        //
        val (updatedSeed, offspring: IndexedSeq[Geneology[A]]) =
        partnerIndices.foldLeft(mateSeed -> IndexedSeq.empty[Geneology[A]]) {
        ...
        val newChildren: IndexedSeq[Geneology[A]] = offspring ++ results
    ...
    //
    val nextGen = distinctBy(newGeneration)(_.value)
      .sortBy(_.value)
      .take(settings.maxPopulationSize)
    (newSeed, nextGen)
  }

  // we've got this in 2.13
  private def distinctBy[A, B](coll: IndexedSeq[A])(f: A => B) = {
    val byKey = coll.foldLeft(Map.empty[B, A]) {
      case (map, next) =>
        val key = f(next)
        if (map.contains(key)) {
          map
  ...
  def combineAndMutate[A: AlgoSettings](
    ...
    val (combineSeed, child) = settings.combine(random, mom.value, dad.value)
